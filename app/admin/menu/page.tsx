...
interface MenuItem {
  ...
  price: string; image_url: string; is_active: boolean;
}

export default function AdminMenuPage() {
  const [items, setItems] = useState<MenuItem[]>([]);
  const [editing, setEditing] = useState<MenuItem | null>(null);
  const [form, setForm] = useState({ category: "", name: "", description: "", price: "", is_active: true });
  ...
  const loadItems = async () => {
    try {
      const r = await fetch(`${API}/api/menu/all`, { credentials: "include" });
      if (!r.ok) throw new Error();
      setItems(await r.json());
      setError("");
    } catch {
      setError("Could not load menu items. Make sure the backend server is running on port 5000.");
    }
  ...
  const openNew = () => { setEditing(null); setForm({ category: "", name: "", description: "", price: "", is_active: true }); setImageFile(null); setImagePreview(""); setShowForm(true); };
  const openEdit = (item: MenuItem) => { setEditing(item); setForm({ category: item.category, name: item.name, description: item.description, price: item.price, is_active: !!item.is_active }); setImageFile(null); setImagePreview(item.image_url || ""); setShowForm(true); };
  ...
  const handleSave = async () => {
    ...
    const url = editing ? `${API}/api/admin/menu/${editing.id}` : `${API}/api/admin/menu`;

    if (editing) {
      try {
        const r = await fetch(url, { method, credentials: "include", body: fd });
        const resp = await r.json();
        setItems((prev) => prev.map((i) => i.id === editing.id ? { ...i, category: form.category, name: form.name, description: form.description, price: form.price, is_active: form.is_active, image_url: resp.image_url || i.image_url } : i));
      } catch {
        setItems((prev) => prev.map((i) => i.id === editing.id ? { ...i, category: form.category, name: form.name, description: form.description, price: form.price, is_active: form.is_active } : i));
      }
      setShowForm(false);
      setSaving(false);
    } else {
      ...
      setItems((prev) => [{ id: data.id, category: form.category, name: form.name, description: form.description, price: form.price, image_url: data.image_url || "", is_active: form.is_active }, ...prev]);
      setShowForm(false);
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    fetch(`${API}/api/admin/menu/${id}`, { method: "DELETE", credentials: "include" });
  };

  return (
    <AdminLayout>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem", flexWrap: "wrap", gap: "1rem" }}>
        <h1 style={{ fontFamily: "var(--font-bebas-neue), sans-serif", fontSize: "2rem", color: "#1b3c33", letterSpacing: "0.05em" }}>Menu Management</h1>
        <button onClick={openNew} style={{ padding: "0.7rem 1.8rem", borderRadius: "100px", border: "none", background: "#1b3c33", color: "#fdf9f4", fontFamily: "var(--font-outfit), sans-serif", fontWeight: 700, fontSize: "0.85rem", cursor: "pointer" }}>+ Add Item</button>
      </div>

      {showForm && (
        <div style={{ background: "#fff", borderRadius: "20px", padding: "2rem", marginBottom: "2rem", boxShadow: "0 4px 24px rgba(27,60,51,0.06)" }}>
          <h2 style={{ fontFamily: "var(--font-bebas-neue), sans-serif", fontSize: "1.3rem", color: "#1b3c33", marginBottom: "1.2rem" }}>{editing ? "Edit Item" : "New Item"}</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "1rem" }}>
            <div><label style={{ display: "block", fontSize: "0.8rem", fontWeight: 700, color: "#586159", marginBottom: "0.3rem", fontFamily: "var(--font-outfit), sans-serif" }}>Category</label><input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} style={adminInputStyle} placeholder="e.g. Hot Coffee" /></div>
            <div><label style={{ display: "block", fontSize: "0.8rem", fontWeight: 700, color: "#586159", marginBottom: "0.3rem", fontFamily: "var(--font-outfit), sans-serif" }}>Name</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={adminInputStyle} placeholder="Item name" /></div>
            <div><label style={{ display: "block", fontSize: "0.8rem", fontWeight: 700, color: "#586159", marginBottom: "0.3rem", fontFamily: "var(--font-outfit), sans-serif" }}>Price</label><input value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} style={adminInputStyle} placeholder="e.g. 199" /></div>
      ...
      {loading ? (
        <p style={{ fontFamily: "var(--font-outfit), sans-serif", color: "#586159", textAlign: "center", padding: "3rem" }}>Loading menu items...</p>
      ) : categories.map((cat) => (
              ...
              <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff", borderRadius: "12px", padding: "1rem 1.2rem", boxShadow: "0 2px 12px rgba(27,60,51,0.04)", opacity: item.is_active ? 1 : 0.5 }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                  ...
                  <div>
                    ...
                    {item.price && <span style={{ fontFamily: "var(--font-outfit), sans-serif", color: "#586159", fontSize: "0.85rem", marginLeft: "0.75rem" }}>&#8377;{item.price}</span>}
                  </div>
                </div>
